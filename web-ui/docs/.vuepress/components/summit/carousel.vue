  ...
  <div class="agenda-carousel" :class="summitData ? 'summit2021' : ''">
    <div class="agenda-msg">
      <div class="time-list" :class="{ summit_time: isShowH5 && summitData }">
        <div
          :id="index == 1 && !summitData ? 'goCenter' : ''"
          v-for="(item, index) in carouselObj.TIME_LIST"
          :key="index"
          :class="{ 'tea-div': summitData && index === 4 }"
        >
          {{ item }}
        </div>
      </div>
      <div class="card-list">
        ...
        <div class="msg-detail" @click="showDetail('1')" v-if="isShowDetail">
          ...
          <div class="desc-box">
            ...
            <p v-for="(item, index) in detailMsg.DESC" :key="index" v-if="item">
              {{ item }}
            </p>
          </div>
        </div>
        <div class="card-box">
          ...
          >
            <li v-for="(item, index) in carouselObj.CARD_LIST" :key="index">
              <p
                class="section"
                v-for="(values, keys) in item.TITLE"
                :key="values"
              >
                {{ values }}
              </p>
              <a
                ...
                :class="[
                  ...
                  { teacard: key == 4 && summitData },
                  { teacard_mo: key == 4 && isShowH5 && summitData },
                ]"
    ...
    <div class="sig-content" v-if="sigObj && !summitData">
      ...
      <div class="sig-2">
        ...
        <div class="detail">
          <div
            v-for="(item, index) in sig2DetailList"
            @click="showDetail(item, 2)"
            :class="item.CONTENT ? '' : 'not-click'"
          >
            {{ item.THEME }}
          </div>
        </div>
    ...
    handleBtn(direction) {
      ...
      if (direction === 'left') {
        if (this.cardIndex <= 0) {
          ...
          this.isShowH5
            ? (this.cardPosition = this.cardPosition + 220)
            : (this.cardPosition = this.cardPosition + 317);
        }
      } else {
...
@media screen and (min-width: 700px) and (max-width: 1000px) {
  // 适配ipad
  .agenda-carousel .agenda-msg .card-list .card-box,
